...
**`frontend/package.json`** : `npm run build` lance maintenant `build:tailwind` (inchangé, pour les pages pas encore migrées) **puis** `build:vite` (nouveau, pour les pages migrées) - un seul `npm run build` continue de tout produire correctement, aucun changement de configuration Netlify nécessaire pour l'instant (le "publish directory" ne changera que lorsque *toutes* les pages seront migrées).

**Fondations partagées** (`frontend/src/react/`) : `providers/AuthProvider.jsx` et `providers/I18nProvider.jsx` portent `assets/js/auth.js`/`i18n.js` en contexts React (mêmes endpoints, même `localStorage`, mêmes dictionnaires JSON `assets/i18n/*.json` réutilisés tels quels) ; `components/Navbar.jsx`/`Footer.jsx` remplacent le HTML dupliqué sur chaque page. **Piège de fidélité à surveiller à chaque page migrée** : `src/input.css` cible certains éléments par ID (`#navbarCollapse`, `#navbarToggler`, `#lang-switch-btn`) - ces IDs doivent être préservés dans le JSX, sinon les règles CSS existantes (dont le fix mobile-dropdown text-color) cessent silencieusement de s'appliquer.

**État** : Lot 0 fait (`404.html`, `login.html`) - preuve de bout en bout que la chaîne Vite/Netlify fonctionne avant d'investir dans les lots suivants (comptes, données, visualisations + Mercure).
