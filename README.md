...
**Déploiement pendant la migration** : le build Netlify a été désactivé (2026-09-01, ressources limitées) - la vérification se fait sur le Codespace (`npm install && npm run build`, servi en local sur le port 8123, backend sur 8080, tous deux en visibilité publique via `gh codespace ports visibility`). Netlify sera reconsidéré maintenant que la migration est terminée.
...
**Hors périmètre pour l'instant** : déploiement du hub en production (Render ne fait tourner qu'un seul service aujourd'hui - point d'attention #38 - un hub Mercure nécessiterait un 2e/3e service déployé, décision séparée côté coût/plan Render) ; Netlify, désactivé depuis le 2026-09-01, sera reconsidéré maintenant que toute la migration (A3.1/A3.2 compris) est terminée.
