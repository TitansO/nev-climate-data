...
### Migration React (A3.1/A3.2, en cours)
...
**`frontend/package.json`** : `npm run build` lance `build:tailwind` (inchangé, pour les pages pas encore migrées) **puis** `build:vite` (nouveau, pour les pages migrées - commence par `rm -rf assets/react` pour ne jamais accumuler de bundles orphelins d'un lot précédent). Un seul `npm run build` continue de tout produire correctement.
